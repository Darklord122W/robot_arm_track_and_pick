...
Brings up:
  - charuco_tf_publisher (camera_color_optical_frame -> handeye_target)
  - easy_handeye2 handeye_server (eye_on_base, world / link2 / camera_color_optical_frame / handeye_target)
  ...
  - dummy world -> camera_color_optical_frame static TF that easy_handeye2's
    own calibrate.launch.py adds for eye_on_base mode (so the TF chain check
    in handeye_server passes; the calibration math itself does not consume
    this dummy).

Run only AFTER hardware driver, MoveIt, and the camera are already up.
...
def generate_launch_description():
    args = [
        ...
        DeclareLaunchArgument('robot_effector_frame', default_value='link2',
                              description='Link the calibration board is rigidly attached to.'),
        ...
        # ChArUco params — defaults match the calib.io 210x150mm 5x7 / 26mm / 19mm board.
    ...
    return LaunchDescription(args + [charuco_node, handeye_server, handeye_rqt, dummy_tf])
